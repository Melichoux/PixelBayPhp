...
if (empty($errors)) {
    $stmt = $pdo->prepare("SELECT id FROM users WHERE email = :email"); // on "annonce" la commande qui va etre utilisée dans la table de la BDD
...
if (empty($errors)) { // dans l'ideal, faire un try catch
    ...
    $stmt = $pdo->prepare("INSERT INTO users (prenom, nom, email, password, age, cgu) VALUES (:nom, :prenom, :email, :password, :age, :cgu)");
    $stmt->execute([
        'nom' => $last_name,
        'prenom' => $first_name,
        'email' => $email,
